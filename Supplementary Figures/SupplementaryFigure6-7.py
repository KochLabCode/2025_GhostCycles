...
for I in tqdm(I_vals, "Simulations for Figure S6b,c"):
    params = [I,v1,v2,v3,v4,gk,gl,gca,vca,vk,vl,phi,cm]
...
if load == False:
    for I in I_range:
        ...
        for i in tqdm(range(A_range.shape[0]),"Simulations for Figure S6d ("+str(I)+")"):
...
area_entr = []
sections = [[0.86,1.15],[0.7,2.7]] # for phase difference plots 
aTs = []
...
#%% Supplementary Figure 6e - response to noise: phase differences  
...
for I in range (2):
    ...
    for i in tqdm(range(len(omega_range)), "Simulations for Figure S6f "+['(slow-fast regime)','(1-ghost cycle'][I]):
...
#%% Supplementary Figure 7 - Timecourses periodically forced ML system
...
m_labels = ['slow-fast regime', '1-ghost cycle']
...
for i in tqdm(range(len(omega_range)), "Simulations for Figure S7"):
