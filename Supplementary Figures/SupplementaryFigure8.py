...
#%% Figure S8a - bifurcation diagram
...
plt.savefig('FigS8a.svg')

#%% Supplementary Figure 8b,c - timescales
...
for pE in tqdm(pE_vals, "Simulations for Figure S8b,c"):
    ...
    plt.figure()
    plt.title('p$_{e}$='+str(pE))
    plt.plot(time,solution.y[0,:],'-', color=colors[i])
    plt.ylabel('V')
    plt.xlabel('time')
    i+=1
...
for e in [0,1]:
    ...
    # time, simDat = fun.RK4(mod.MorrisLecar,params,eC,0,dt,t_end+t_tr)
...
plt.savefig('FigS8b,c.svg')


#%% Supplementary Figure 8d - Arnold tongues
...
if load == False:
    for e in pe_range:
        ...
        for i in tqdm(range(A_range.shape[0]),"Simulations for Figure S8d ("+str(e)+")"):
            print(e,i)
...
area_entr = []
sections = [[0.75,1.2],[0.1,2.8]] # for phase difference plots 
aTs = []
...
#%% Supplementary Figure 8e - response to noise: phase differences  

load = True

np.random.seed(0)
sigma_range = [0,1e-7,1e-5]
A=0.003
nVec = np.array([0,1])
    ...
    for e in [0,1]:
        ...
        for sig in sigma_range:
            ...
            for j in range(omega_range.shape[0]):
                ...
                #transient phase
                time, simDat = fun.RK4_na_noisy(wrapper,params,np.array([0.1,0.1]),-T0[e],dt,0,nVec, sig, naFun = None,naFunParams = None)
                #post transient
...
for e in [0,1]:
    ...
    plt.savefig('FigS8e_'+str(e)+'.svg')
    

#%% Supplementary Figure 8f - ISI histograms
...
for e in range (2):
    ...
    for i in tqdm(range(len(omega_range)), "Simulations for Figure S8f "+['(slow-fast regime)','(1-ghost cycle'][I]):
    ...
    plt.savefig('FigS8f_'+str(e)+'.png', dpi=400)
    plt.savefig('FigS8f_'+str(e)+'.svg')