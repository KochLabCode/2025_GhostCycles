...
#%% Supplementary Figures 3-4 - timecourses of periodically forced VdP/VdP_1g/VdP_2g systems
...
for m in [0,1]:
    ...
    t_end = 10*T0[m]; npts = int(t_end/dt); time = np.linspace(0,t_end,npts+1)  
    
    fold_unforced = np.logspace(-1,1,20)
    omega_range = fold_unforced*2*np.pi/T0[m]
    
    plt.figure(figsize=(17.2*inCm,18*inCm))
    plt.suptitle(lbls[m] + ', A = ' + str(A), fontsize=10)
    
    for i in range(len(omega_range)):
        print(i)
        # print('Supplementary Figure '+str(m+2)+': simulation '  +str(100*i/len(omega_range)) + '% complete')
        ...
        LLE = fun.maxLyap('LSODA', models_aut_trans[m], para[m], np.array([0.1,0.1,0]), jacobians[m], max(20*T0[m], 500), dt, max(5*T0[m], 100))
    ...
    plt.tight_layout()
    plt.savefig('FigureS'+str(3+m)+'.svg')
    

#%% Supplementary Figure 5 - potential VdP vs VdP2G
